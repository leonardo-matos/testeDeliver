    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function __construct($connection, $config = array()){
	    ...
	    $this->config = array_merge(array(
            'client_table' => 'api.oauth_clients',
            'access_token_table' => 'api.oauth_access_tokens',
            'refresh_token_table' => 'api.oauth_refresh_tokens',
            'code_table' => 'api.oauth_authorization_codes',
            'user_table' => 'api.oauth_users',
            'jwt_table'  => 'api.oauth_jwt',
            'jti_table'  => 'api.oauth_jti',
            'scope_table'  => 'api.oauth_scopes',
            'public_key_table'  => 'api.oauth_public_keys',
        ), $config);
    ...
    public function checkUserCredentials($username, $password){
        ...
        if ($user = $this->getUser($username)) {
         ...
         $aux = $this->checkPassword($user, $password);
    ...
    /**
         * Aterado a função nativa devido erro de login App.
         * 
         * @param $username
         * 
         * 
    ...
    protected function checkPassword($user, $password) {        
    	...
    	if(!oci_fetch_assoc($stmt)){
			// return true; //colocar como true para aceitar qualquer senha no login do APP(somente para testes)
			return false;
    	}
    ...
    public function getBuildSql($dbName = 'oauth2_server_php'){
        ...
        /**
         * Função Para gerar a senha para a acesso para pegar cod_pessoa
         * 
         * @param $username
         * $this->getEncryptedPassword($user['username'],$password));
         * 
         ...
         * 
         */
    public function getUserCodePergamum( $username, $password ) {
        $sql = 'select * from '.$this->config['user_table']." where username='".$username."'";
        $stmt = oci_parse($this->db, $sql);
        oci_execute($stmt);
        $userInfo = @array_change_key_case(oci_fetch_assoc($stmt),CASE_LOWER);
        ...
        if($userInfo['username'] != $userInfo['cod_pessoa']){
            $retorno = array(
            'code' => $userInfo['cod_pessoa'],
            'value' => $password
             );
        } else {
            $retorno = array(
   ...
   public function getEncryptedPassword( $username, $password ) {
    	// Busca a senha (criptografada).
    	// $code = $this->getUserCode();
        $userCodePergamum = $this->getUserCodePergamum( $username, $password );
        // $code = '4821';
        $code = $userCodePergamum['code'];
        $value = $userCodePergamum['value'];
