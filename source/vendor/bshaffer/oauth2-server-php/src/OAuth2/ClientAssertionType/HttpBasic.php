...
class HttpBasic implements ClientAssertionTypeInterface
{
    ...
    public function validateRequest(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if (!isset($clientData['client_secret']) || $clientData['client_secret'] == '') {
            ...
            if (!$this->config['allow_public_clients']) {
                $response->setError(400, 400, 'As credenciais do cliente são necessárias(client credentials)');

                return false;
            }

            if (!$this->storage->isPublicClient($clientData['client_id'])) {
                $response->setError(400, 400, 'Este "client_id" não é válido ou deve ser autenticado usando uma "client_secret"');
            ...
            $response->setError(400, 400, 'As credenciais do cliente não são válidas(client_id ou client_secret incorretas) ');
    ...
    public function getClientCredentials(RequestInterface $request, ResponseInterface $response = null)
    {
        ...
        if ($response) {
            ...
            $response->setError(400, 400, 'As credenciais do cliente não foram encontradas no cabeçalho HTTP'.$message);
        }
