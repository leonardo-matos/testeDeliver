...
class ResourceController implements ResourceControllerInterface
{
    ...
    public function verifyResourceRequest(RequestInterface $request, ResponseInterface $response, $scope = null)
    {
        ...
        if ($scope && (!isset($token["scope"]) || !$token["scope"] || !$this->scopeUtil->checkScope($scope, $token["scope"]))) {
            $response->setError(403, 403, 'A requisição necessita privilégios maiores do que o providenciado pelo access token','http://tools.ietf.org/html/rfc6750#section-3.1');
            $response->addHttpHeaders(array(
                'WWW-Authenticate' => sprintf('%s realm="%s", scope="%s", error="%s", error_description="%s"',
                    ...
                    $scope,
                    $response->getParameter('codigo'),
                    $response->getParameter('mensagem')
                )
            ));
    ...
    public function getAccessTokenData(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if ($token_param = $this->tokenType->getAccessTokenParameter($request, $response)) {
            ...
            if (!$token = $this->tokenStorage->getAccessToken($token_param)) {
                $response->setError(401, 401, 'O access token informado não é válido');
            } elseif (!isset($token["expires"]) || !isset($token["client_id"])) {
                $response->setError(401, 401, 'Token formado incorretamente (faltando parametro "expires")');
            } elseif (time() > $token["expires"]) {
                $response->setError(401, 401, 'O access token informado está expirado');
            } else {
                return $token;
            }
        }
