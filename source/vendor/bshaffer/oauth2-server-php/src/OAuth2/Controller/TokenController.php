...
class TokenController implements TokenControllerInterface
{
    ...
    public function grantAccessToken(RequestInterface $request, ResponseInterface $response)
    {
        if (strtolower($request->server('REQUEST_METHOD')) != 'post') {
            $response->setError(405, 405, 'O m�todo de requisi��o deve ser POST ao requisitar um access token', 'https://tools.ietf.org/html/draft-ietf-oauth-v2-05#section-3.8');
        ...
        if (!$grantTypeIdentifier = $request->request('grant_type')) {
            $response->setError(400, 400, 'O tipo de concess�o (grant_type) n�o foi especificado na requisi��o');
        ...
        if (!isset($this->grantTypes[$grantTypeIdentifier])) {
            ...
            $response->setError(400, 400, sprintf('Tipo de concessao(grant_type) "%s" n�o suportado', $grantTypeIdentifier));
        ...
        if ($grantType instanceof ClientAssertionTypeInterface) {
            ...
            if (!is_null($storedClientId = $grantType->getClientId()) && $storedClientId != $clientId) {
                $response->setError(400, 400, sprintf('%s n�o existe ou � invalido para o cliente', $grantTypeIdentifier));
        ...
        // var_dump('$grantTypeIdentifier: ',$grantTypeIdentifier);
        // var_dump(!$this->clientStorage->checkRestrictedGrantType($clientId, $grantTypeIdentifier));exit;
        if (!$this->clientStorage->checkRestrictedGrantType($clientId, $grantTypeIdentifier)) {
            $response->setError(400, 400, 'O tipo de concess�o (grant_type) n�o est� permitido para este client_id');
        ...
        if ($requestedScope) {
            ...
            if ($availableScope) {
                if (!$this->scopeUtil->checkScope($requestedScope, $availableScope)) {
                    $response->setError(400, 400, ' O escopo(scope) solicitado n�o � v�lido para esta requisi��o');
                ...
                if ($clientScope = $this->clientStorage->getClientScope($clientId)) {
                    if (!$this->scopeUtil->checkScope($requestedScope, $clientScope)) {
                        $response->setError(400, 400, 'O escopo(scope) solicitado � inv�lido para este cliente');

                        return false;
                    }
                } elseif (!$this->scopeUtil->scopeExists($requestedScope)) {
                    $response->setError(400, 400, 'Um escopo(scope) n�o suportado foi solicitado ');
            ...
            if (false === $defaultScope) {
                $response->setError(400, 400, 'Esta aplica��o requer que voc� especifique o parametro "scope"');
    ...
    {
        if (strtolower($request->server('REQUEST_METHOD')) != 'post') {
            $response->setError(405, 405, 'O m�todo de requisi��o deve ser POST ao revogar um access token', 'https://tools.ietf.org/html/rfc6749#section-3.2');
            $response->addHttpHeaders(array('Allow' => 'POST'));
        ...
        if (!in_array($token_type_hint, array(null, 'access_token', 'refresh_token'), true)) {
            $response->setError(400, 'requisicao_invalida', 'A pista do tipo de token(token_type_hint) deve ser um \'access_token\' ou \'refresh_token\'');
        ...
        if ($token === null) {
            $response->setError(400, 405,'Esta faltando o parametro \'token\' para ser poss�vel revogar o access token');
