...
class AuthorizeController implements AuthorizeControllerInterface
{
    ...
    public function validateAuthorizeRequest(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if (!$client_id = $request->query('client_id', $request->request('client_id'))) {
            ...
            $response->setError(400, 400, "O 'client_id' não foi informado");
        ...
        if (!$clientData = $this->clientStorage->getClientDetails($client_id)) {
            $response->setError(400, 400, "O 'client_id' informado não é valido");
        ...
        if ($supplied_redirect_uri = $request->query('redirect_uri', $request->request('redirect_uri'))) {
            ...
            if (isset($parts['fragment']) && $parts['fragment']) {
                $response->setError(400, 400, 'A URI de redirecionamento(redirect_uri) não contem um fragmento(fragment)','http://tools.ietf.org/html/rfc6749#section-3.1.2');
            ...
            if ($registered_redirect_uri && !$this->validateRedirectUri($supplied_redirect_uri, $registered_redirect_uri)) {
                $response->setError(400, 400, 'A URI de redirecionamento(redirect_uri) informada está faltando ou não corresponde', 'https://tools.ietf.org/html/rfc6749#section-3.1.2');
            ...
            if (!$registered_redirect_uri) {
                $response->setError(400, 400, 'Nenhuma URI de redirecionamento(redirect_uri) foi informada ou está armazenada');

                return false;
            }

            if (count(explode(' ', $registered_redirect_uri)) > 1) {
                $response->setError(400, 400, 'A URI de redirecionamento(redirect_uri) deve ser informada quando multiplas URIs estão registradas', 'https://tools.ietf.org/html/rfc6749#section-3.1.2.3');
