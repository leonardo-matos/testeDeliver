...
class AuthorizationCode implements GrantTypeInterface
{
    ...
    public function validateRequest(RequestInterface $request, ResponseInterface $response)
    {
        if (!$request->request('code')) {
            $response->setError(400, 400, 'Parâmetros faltando: "code" é obrigatório');
        ...
        if (!$authCode = $this->storage->getAuthorizationCode($code)) {
            $response->setError(400, 400, 'O código de autorização não existe ou é invalido para este cliente');
        ...
        if (isset($authCode['redirect_uri']) && $authCode['redirect_uri']) {
            if (!$request->request('redirect_uri') || urldecode($request->request('redirect_uri')) != $authCode['redirect_uri']) {
                $response->setError(400, 400, "A URI de redirecionamento está faltando ou não combinou com a que está armazenada", "#section-4.1.3");
        ...
        if ($authCode["expires"] < time()) {
            $response->setError(400, 400, "O codigo de autorização expirou");
